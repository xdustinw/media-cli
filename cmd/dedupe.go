...
Which copy is kept (-k / --keep):
  ...
  f<n> / folder<n> protect the n-th folder given here: its copies are never
                   deleted, and a duplicate in any other folder is (a set with
                   no copy in that folder is left alone). e.g. -k f1
...
anything is removed, unless -y.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		keep, err := dedupecmd.ParseKeep(flagDedupeKeep)
		if err != nil {
			return err
		}
...
func init() {
	dedupeCmd.Flags().StringVarP(&flagDedupeKeep, "keep", "k", "",
		"which copy to keep: interactive (default), longer-name, newer, older, f<n> (i|l|n|o|f1|f2…)")
